...
test('state 와 검증값은 추측할 수 없을 만큼 길다', () => {
  ...
  const { state, verifier } = newAttempt()
...
test('짝은 짧게 산다', () => {
  ...
  const { createdAt, expiresAt } = newAttempt()
...
test('authorize 주소에 state 와 등록한 redirect 가 실린다', () => {
  ...
  assert.equal(url.searchParams.get('client_id'), '클라이언트id')
...
test('authorize 주소에 client_secret 이 안 실린다', () => {
  ...
  const url = authorizeUrl(newAttempt().state)
  assert.equal(url.includes('비밀'), false)
  assert.equal(url.includes('client_secret'), false)
})

test('프렌즈 스코프 넷을 요청한다', () => {
  // 이 넷이 Open ID 로 열리는 전부다. 안 받으면 로그인은 되는데 아무것도 못 읽는다.
  const scope = new URL(authorizeUrl(newAttempt().state)).searchParams.get('scope') ?? ''

  for (const 하나 of ['characterlist', 'achievement', 'cube', 'scheduler']) {
...
test('code 를 토큰으로 바꾼다', async () => {
  ...
  const 받은것 = await exchangeCode('받은code', 넥슨.fetcher)
...
test('액세스 토큰 만료 시각을 넥슨이 준 초로 잰다', async () => {
  ...
  const 받은것 = await exchangeCode('code', 넥슨.fetcher, 지금)
...
test('넥슨이 거절하면 던진다', async () => {
  ...
  await assert.rejects(() => exchangeCode('이미쓴code', 넥슨.fetcher))
})
...
test('토큰이 빠진 응답도 던진다', async () => {
  ...
  await assert.rejects(() => exchangeCode('code', 넥슨.fetcher))
})
...
test('갱신은 갱신 토큰으로 부른다', async () => {
  ...
  await refreshTokens('옛갱신토큰', 넥슨.fetcher)
  const 보낸 = new URLSearchParams(넥슨.보낸것.body ?? '')
...
test('갱신이 새 갱신 토큰을 안 주면 옛것을 그대로 쓴다', async () => {
  ...
  const 받은것 = await refreshTokens('옛갱신토큰', 넥슨.fetcher)
